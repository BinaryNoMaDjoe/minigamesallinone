...
- **字标内容（本项站点专属）**：站名 `minigamesallinone.binarynomad.io`。建议主标 **"MINIGAMES ALL IN ONE"**（display/headline 大写斜体，可拆行 "MINIGAMES / ALL IN ONE"），机构副标 **"BINARY NOMAD"** 用 `label-bold` 置于字标旁或页脚。最终排法见 §14 遗留待定
