...
> **版本**：v0.8（主站排版修正：Hero 拼贴面板响应式尺寸 + 竖版面板封面 contain 保字标完整，决策 #28）
...
- Hero 面板拼贴：md 以下隐藏多面板组，改为居中展示单张倾斜小面板（约 `w-40 h-52`，`rotate-3`，红色硬阴影）；md 以上拼贴面板尺寸随容器等比缩放（侧板 45% / 主卡 56% 宽，竖版比例 2:3 与 32:45，min() 封顶 256px / 320px；容器高 md 400px、lg 500px），杜绝固定像素面板在窄 md 视口溢出被裁（决策 #28）
...
- **图区封面**（决策 #26）：由游戏 manifest.cover（游戏目录内 `cover.svg` 等资源 URL）渲染；未提供封面时回退为半调网点 + 游戏名首字占位。Hero 面板同样优先使用 cover
  - 卡片头图（近 4:3 横版图区）：`object-cover` 铺满图区
  - Hero 拼贴面板 / 移动端单面板（2:3 竖版）：封面 `object-contain` + 半调衬底，**不得铺满裁切**——横版封面在竖版面板上铺满会切掉两侧字标（决策 #28）
- 文字区：`p-6` + 半调网点底
...
- 图片比例：卡片头图约 4:3；Hero 面板竖版约 2:3。**单一横版封面在竖版面板上不得铺满**：Hero 面板与移动单面板用 `object-contain` + 半调衬底（保证封面字标完整）；卡片头图维持 `object-cover` 铺满（决策 #28）
...
| 28  | 主站排版修正 | Hero 拼贴面板尺寸响应式（比例 + min() 封顶，消除 md 溢出裁切）；竖版面板封面 object-contain 半调衬底保证字标完整，卡片头图维持 object-cover（§4.3/§8.2/§11） | ✅ 已定（主表） |
