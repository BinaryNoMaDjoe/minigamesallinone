...
> **版本**：v0.6（封面缩略图机制：§8.2 图区封面，决策 #26）
...
> **变更记录**：v0.1 初稿 → v0.2 依据评审决议生长（暗色主题 §2.5、中文字体选型 §3.4、品牌字标 §8.3、弹窗运行模式 §15）；§13 落地映射对齐脚手架决策（Tailwind v4、字体自托管）→ v0.3 依据用户指令生长（2026-08-15）：响应式扩写 §4.3（横竖屏硬约束）、新增欢迎页 §8.10、动效 §9 新增滚动入场与欢迎页两行 → v0.4 依据用户指令（2026-08-15）：字标内容定案（§8.3/§8.10/§14，决策 #22）→ v0.5（2026-08-15）：§15 控制条随游戏阶段适配（ADR-0007、决策 #24）→ v0.6（2026-08-15）：§8.2 图区封面机制（决策 #26）。
