...
> **版本**：v0.4（字标决议：MINIGAMESALLINONE / by@BinaryNomad.io）
...
> **变更记录**：v0.1 初稿 → v0.2 依据评审决议生长（暗色主题 §2.5、中文字体选型 §3.4、品牌字标 §8.3、弹窗运行模式 §15）；§13 落地映射对齐脚手架决策（Tailwind v4、字体自托管）→ v0.3 依据用户指令生长（2026-08-15）：响应式扩写 §4.3（横竖屏硬约束）、新增欢迎页 §8.10、动效 §9 新增滚动入场与欢迎页两行 → v0.4 依据用户指令（2026-08-15）：字标内容定案（§8.3/§8.10/§14，决策 #22）。
...
- **出框图（Pop-out）**：Hero 图可突破卡片顶边（z-index 溢出），图区底部用 4px 墨线分隔
- 文字区：`p-6` + 半调网点底
...
5. **控制条**：当前分/最高分（`label-bold`）、暂停、重新开始（§8.1 Icon 变体，40px 方形）、关闭（窗口右上角 40px 方形图标钮）
