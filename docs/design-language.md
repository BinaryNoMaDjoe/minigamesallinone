...
> **版本**：v0.9（主站排版修正修订：Hero 面板改用竖版封面 coverPortrait 铺满——无留白、无裁切，决策 #28）
...
> **变更记录**：v0.1 初稿 → v0.2 依据评审决议生长（暗色主题 §2.5、中文字体选型 §3.4、品牌字标 §8.3、弹窗运行模式 §15）；§13 落地映射对齐脚手架决策（Tailwind v4、字体自托管）→ v0.3 依据用户指令生长（2026-08-15）：响应式扩写 §4.3（横竖屏硬约束）、新增欢迎页 §8.10、动效 §9 新增滚动入场与欢迎页两行 → v0.4 依据用户指令（2026-08-15）：字标内容定案（§8.3/§8.10/§14，决策 #22）→ v0.5（2026-08-15）：§15 控制条随游戏阶段适配（ADR-0007、决策 #24）→ v0.6（2026-08-15）：§8.2 图区封面机制（决策 #26）→ v0.7（2026-08-15，用户指令）：§4.3 响应式细化——弹窗公式 dvh 化（回退 vh、负值失效回退全宽）、触控目标 ≥40px 落地细则（含窄窗宽度收窄例外）、游戏内窄窗容器查询紧凑布局（决策 #27）→ v0.8（2026-08-15，用户指令）：§4.3/§8.2/§11 主站排版修正——Hero 拼贴面板尺寸响应式（比例 + min() 封顶，消除 md 溢出裁切）、竖版面板与移动单面板封面改 object-contain 半调衬底（杜绝字标裁切；卡片头图维持 object-cover）、修正 §11 图片比例说明（决策 #28）→ v0.9（2026-08-15，用户指令）：修订 v0.8 竖版面板封面方案——object-contain 上下留白被否，改为 manifest 新增 **coverPortrait 竖版封面**（2:3 渲染），Hero 面板/移动单面板铺满无留白无裁切；卡片头图维持横版 cover；§8.2/§11/§14 同步修订（决策 #28 修订）。
...
- Hero 面板拼贴：md 以下隐藏多面板组，改为居中展示单张倾斜小面板（约 `w-40 h-52`，`rotate-3`，红色硬阴影）；md 以上拼贴面板尺寸随容器等比缩放（侧板 45% / 主卡 56% 宽，竖版比例 2:3 与 32:45，min() 封顶 256px / 320px；容器高 md 400px、lg 500px），杜绝固定像素面板在窄 md 视口溢出被裁；面板封面用竖版封面（§8.2）铺满，无留白无裁切（决策 #28）
...
- **图区封面**（决策 #26、#28）：未提供封面时回退为半调网点 + 游戏名首字占位。封面分两个方向、成对提供：
  - 横版封面 `manifest.cover`（约 3:2）：卡片头图（近 4:3 横版图区）`object-cover` 铺满图区
  - 竖版封面 `manifest.coverPortrait`（2:3）：Hero 拼贴面板 / 移动端单面板 `object-cover` 铺满——**无留白、无裁切**；未提供竖版封面的游戏回退横版封面 `object-contain` + 半调衬底（禁止在竖版面板上铺满横版封面，会切掉两侧字标）
- 文字区：`p-6` + 半调网点底
...
- 图片比例：卡片头图约 4:3（横版封面）；Hero 面板竖版约 2:3（竖版封面）。**两个方向各渲染一张封面**：横版 `cover` 与竖版 `coverPortrait`（2:3）成对提供；竖版面板铺满横版封面必裁两侧字标，横版面板铺满竖版封面必裁上下——一律按方向使用对应封面（决策 #28）
...
| 28  | 主站排版修正 | Hero 拼贴面板尺寸响应式（比例 + min() 封顶，消除 md 溢出裁切）；manifest 新增 coverPortrait 竖版封面（2:3 渲染）供 Hero 面板/移动单面板铺满（无留白无裁切），卡片头图维持横版 cover 铺满（§4.3/§8.2/§11） | ✅ 已定（主表） |
